...
import { sampleHouseholds, formatCurrency, formatFullCurrency } from "@/data/sampleData";

const totalAUM = sampleHouseholds.reduce((sum, h) => sum + h.totalAUM, 0);
const totalHouseholds = sampleHouseholds.length;

const upcomingReviews = sampleHouseholds
  .filter((h) => {
    const d = new Date(h.annualReviewDate);
    const now = new Date();
    const diff = (d.getTime() - now.getTime()) / (1000 * 60 * 60 * 24);
    return diff >= 0 && diff <= 60;
  })
  .sort((a, b) => new Date(a.annualReviewDate).getTime() - new Date(b.annualReviewDate).getTime());

const recentActivity = sampleHouseholds
  .flatMap((h) => h.complianceLog.map((n) => ({ ...n, householdName: h.name, householdId: h.id })))
...
export default function Dashboard() {
  return (
    <div className="p-6 lg:p-10 max-w-6xl">
      <div className="mb-8">
        <h1 className="text-2xl font-semibold tracking-tight text-foreground">Good morning, Sarah</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        ...
        <Card className="border-border shadow-none">
          <CardContent className="pt-6">
            ...
            <p className="text-xs text-muted-foreground mt-2">{sampleHouseholds.filter(h => h.status === "Active").length} active</p>
          </CardContent>
        </Card>
            ...
            {upcomingReviews.map((h) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">{formatCurrency(h.totalAUM)} AUM</p>
                </div>
                <div className="flex items-center gap-2">
                  <div className="text-right">
                    <p className="text-xs font-medium text-foreground">
                      {new Date(h.annualReviewDate).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                    </p>
                  </div>
                  ...
                  <div key={note.id} className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <Link to={`/household/${note.householdId}`} className="text-sm font-medium text-foreground hover:underline">
                          {note.householdName}
                        </Link>
